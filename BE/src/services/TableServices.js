...
export const getTableByQR = async (request, reply) => {
  try {
    const qrCode = request.params.qr
    const table = await tableRepo.getTableByQRCode(qrCode)
    if (!table) {
...
export const createTable = async (request, reply) => {
  try {
    ...
    if (!data.qrCode) {
      data.qrCode = `QR-TABLE-${data.tableNumber}-${randomUUID().slice(0, 8)}`
    }
    ...
    if (error.code === 11000) {
      ...
      if (error.keyPattern.qrCode) {
        return reply.code(409).send({ error: 'QR code already exists' })
      }
    }
...
export const getAvailableTables = async (request, reply) => {
  try {
    const tables = await tableRepo.getAvailableTables()
    return tables
  } catch (error) {
    return reply.code(500).send({ error: 'Error retrieving available tables' })
  }
}

export const getOccupiedTables = async (request, reply) => {
  try {
    const tables = await tableRepo.getOccupiedTables()
    return tables
  } catch (error) {
    return reply.code(500).send({ error: 'Error retrieving occupied tables' })
  }
}
...
export const updateTableStatus = async (request, reply) => {
  try {
    ...
    if (!status || !['disponible', 'ocupada'].includes(status)) {
      return reply.code(400).send({ 
        error: 'Invalid status. Must be "disponible" or "ocupada"' 
      })
    }

    const updatedTable = await tableRepo.updateTable(idIncremental, { status })
    if (!updatedTable) {
      return reply.code(404).send({ error: 'Table not found' })
    }
    ...
    return reply.code(500).send({ error: 'Error updating table status' })
  }
}